...
const questionDecide = [
  {
    ...
    choices: [
      "Create entirely new sporting Event.",
...
const questionModify = [
  ...
  },
  {
    ...
    choices: ["Football", "Baseball", "Basketball", "Soccer"],
  },
  {
    type: "input",
    name: "date",
...
const reAskQ = () => {
  inquirer.prompt(questionDecide).then((ans2) => {
    if (ans2.decision === "Modify an event already scheduled.") {
      inquirer.prompt(questionModify).then(async (ans) => {
        let tempID = ans.id;
        delete ans.id;
      ...
      inquirer.prompt(questionDelete).then(async (ans) => {
        let tempID = ans.id;
        let passData = await axios.get(
          `http://localhost:${PORT}/sport/${tempID}`
        );
      ...
      inquirer.prompt(questionCreate).then(async (ans) => {
